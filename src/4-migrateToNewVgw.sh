# This script will perform the migration activity -
# Detach the Old VGW, Attach New VGW, Enable Route Propagation if it was enabled, Update manually entered VGW routes
# INPUT - NONE
# OUTPUT - New VGW attached to the VPC, Route Tables updated
# ERROR CHECKS - NONE
...
# Declare the variables to store route tables, routes, and VPC Id
declare -a routeTables
declare -a routeTablesRP
declare -a routeTablesNoRP
declare -a routes
vpc=`aws ec2 describe-vpn-gateways --vpn-gateway-ids $oldVgw --region $region | grep VpcId | cut -d":" -f2 | sed 's/"//g' | tr -s " " | cut -d" " -f2`
...
# Detach the old VGW and attach the new VGW
# Wait for old VGW to detach and new VGW to attach, before going further
# If the old VGW is already detached or does not exist any more, log and proceed further
# Two checks for detached Old VGW:
...
isDetached1=`aws ec2 describe-vpn-gateways --vpn-gateway-ids $oldVgw --region $region | grep "detached" | wc -l`
isDetached2=`aws ec2 describe-vpn-gateways --vpn-gateway-ids $oldVgw --region $region | grep -E "\"VpcAttachments\": \[\]," | wc -l`
if (( $isDetached1 == 1 || $isDetached2 == 1 )); then
 echo ""
 echo "Old VGW $oldVgw is already detached"
else
 echo ""
 echo "Detaching old VGW $oldVgw from VPC $vpc"
 `aws ec2 detach-vpn-gateway --vpn-gateway-id $oldVgw --vpc-id $vpc --region $region`
 while [ $isDetached1 -lt 1 ]; do
  sleep 30
  isDetached1=`aws ec2 describe-vpn-gateways --vpn-gateway-ids $oldVgw --region $region | grep "detached" | wc -l`
 done
 echo "Old VGW $oldVgw detached"
 echo "" >> $migData
 echo `date` >> $migData
 echo "Old VGW $oldVgw detached" >> $migData
fi

# If the new VGW is already attached, log and proceed further
# If not, attach the new VGW
isAttached=`aws ec2 describe-vpn-gateways --vpn-gateway-ids $newVgw --region $region | grep "attached" | wc -l`
if [ $isAttached == 1 ]; then
 echo ""
 echo "New VGW $newVgw is already attached"
else
 echo ""
 echo "Attaching new VGW $newVgw to VPC $vpc"
 `aws ec2 attach-vpn-gateway --vpn-gateway-id $newVgw --vpc-id $vpc --region $region &> /dev/null`
 while [ $isAttached -lt 1 ]; do
  sleep 5
  isAttached=`aws ec2 describe-vpn-gateways --vpn-gateway-ids $newVgw --region $region | grep "attached" | wc -l`
 done
 echo "New VGW $newVgw attached"
 echo "" >> $migData
 echo `date` >> $migData
 echo "New VGW $newVgw attached" >> $migData
 echo ""
fi

# Enable RP in the route tables which had RP enabled with the old VGW
...
while [ $rpCounter -lt $numRpRt ]; do
 echo "Enabling route propagation in the route-table ${routeTablesRP[$rpCounter]} with VGW $newVgw"
 `aws ec2 enable-vgw-route-propagation --route-table-id ${routeTablesRP[$rpCounter]} --gateway-id $newVgw --region $region`
...
# If RP is disabled, check the routes with Target as old VGW, and replace the Target with new VGW
noRpCounter=0
...
while [ $noRpCounter -lt $numNoRpRt ]; do
 echo "Checking route-table ${routeTablesNoRP[$noRpCounter]} for manually entered routes with Target as $oldVgw"
 routes=(`aws ec2 describe-route-tables --route-table-ids ${routeTablesNoRP[$noRpCounter]} --region $region | grep -A1 $oldVgw | grep DestinationCidrBlock | cut -d":" -f2 | sed "s/,//g" | sed 's/"//g'`)
 numRoutes=${#routes[@]}
 rcounter=0
 echo "Replacing the Target $oldVgw with $newVgw"
 while [ $rcounter -lt $numRoutes ]; do
  `aws ec2 replace-route --route-table-id ${routeTablesNoRP[$noRpCounter]} --destination-cidr-block ${routes[$rcounter]} --gateway-id $newVgw --region $region`
  rcounter=$[$rcounter + 1]
 done
